...
class ConvolutionalOccupancyNetwork(nn.Module):
    ...
    def __init__(self, decoders, encoder=None, device=None, detach_tsdf=False):
        ...
        self.decoder_affrdnce = decoders[1].to(device)
        if len(decoders) > 2:
            self.decoder_tsdf = decoders[2].to(device)
            self.decoder_sem = None
            if len(decoders) > 3:
                self.decoder_sem = decoders[3].to(device)
                if len(decoders) > 4:
    ...
    def forward(self, inputs, grasp_query, p_tsdf=None, sample=True, **kwargs):
        ''' Performs a forward pass through the network.

        Args:
            p (tensor): sampled points, B*N*C
            inputs (tensor): conditioning input, B*N*3
            grasp_query (tuple of tensors): grasp query (ps, rot and local cloud)
        ...
        qual, affrdnce = self.decode(grasp_query, c)
        if p_tsdf is not None:
            if self.detach_tsdf:
                for k, v in c.items():
                    c[k] = v.detach()
            tsdf = self.decoder_tsdf(p_tsdf, c, **kwargs)
            if self.decoder_sem is not None:
                sem = self.decoder_sem(p_tsdf, c, **kwargs)
                return qual, affrdnce, tsdf, sem
            else:
                return qual, affrdnce, tsdf
        else:
            return qual, affrdnce
    ...
    def infer_sem(self, p, inputs, encoded_inputs=False):
        if encoded_inputs == False:
            # encode inputs
            c = self.encode_inputs(inputs)
        else:
        ...
        sem = self.decode_sem(p, c)
        return torch.sigmoid(sem.logits)
    ...
    def decode_sem(self, p, c, **kwargs):
        ''' Returns semantic class probabilities for the sampled points.
        Args:
            p (tensor): points
            c (tensor): latent conditioned code c
        '''

        if self.decoder_sem is None:
            raise ValueError('No semantic decoder available.')
        
        logits = self.decoder_sem(p, c, **kwargs)
        # convert logits to multi class probabilities
        p_r = dist.Categorical(logits=logits)
