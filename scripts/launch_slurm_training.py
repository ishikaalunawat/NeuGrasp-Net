...
launcher = Launcher(
    exp_name='train_neural_grasp_AFFNET',
    exp_file='train_neu_grasp_affnet', # local path without .py
...
launcher.add_experiment(
    net="neu_grasp_pn_affnet_sem",
    net_with_grasp_occ=True, # Don't pass if not True
    # no_handover=True, # Don't pass if not True
    logdir="/work/scratch/sj93qicy/potato-net/runs",
    dataset="/work/scratch/sj93qicy/potato-net/data/3DGraspAff/data_affnet_train_constructed_GPG_60",
    dataset_raw="/work/scratch/sj93qicy/potato-net/data/3DGraspAff/data_affnet_train_random_raw_GPG_60",
    epochs=35,
    batch_size=64,#32,#16,
    num_workers=43,#32,#10,
    lr=2e-4,#1e-4,
    epoch_length_frac=0.325,
    val_split=0.0325,
    description="AFFNET_SEM_v4_balanced",
